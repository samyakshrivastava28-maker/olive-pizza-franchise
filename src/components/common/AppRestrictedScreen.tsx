...
import { ShieldAlert, LogOut, RefreshCw, Building2 } from 'lucide-react';
...
export const AppRestrictedScreen: React.FC<AppRestrictedScreenProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#070A10] text-slate-100 flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-[#0C1220] border border-amber-900/40 rounded-3xl p-6 sm:p-8 shadow-2xl relative z-10 text-center space-y-6">
        <div className="flex justify-center">
          <AppLogo variant="full" size="md" subtitle="Franchise Portal" />
        </div>

        <div className="w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-400 mx-auto flex items-center justify-center shadow-lg shadow-amber-950/30">
          <ShieldAlert className="w-8 h-8" />
        </div>

        <div className="space-y-2">
          <span className="inline-block px-3 py-1 rounded-full text-[11px] font-bold tracking-wider uppercase bg-amber-500/10 text-amber-400 border border-amber-500/20">
            Access Restricted
          </span>
        ...
        <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 text-left text-xs text-slate-400 space-y-2">
          <div className="flex items-center gap-1.5 text-slate-200 font-semibold text-xs">
            ...
            <span>Franchise Security Policy:</span>
          </div>
          <ul className="space-y-1.5 text-[11px] text-slate-400 list-disc list-inside">
            <li>Franchise consoles are restricted strictly to authorized franchise partners and executive management.</li>
            <li>Customer and regular staff accounts cannot access multi-branch franchise metrics or zone controls.</li>
            <li>If you are a franchise owner, ask the Master Owner to grant access to your franchise unit.</li>
          </ul>
        </div>
