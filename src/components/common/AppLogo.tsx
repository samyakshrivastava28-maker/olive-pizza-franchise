...
export const AppLogo: React.FC<AppLogoProps> = ({
  ...
  const renderIcon = () => (
    <div className={`${currentSize.icon} rounded-xl bg-gradient-to-br from-amber-950/80 to-slate-900 border border-amber-500/40 flex items-center justify-center shrink-0 shadow-md p-1`}>
      <svg viewBox="0 0 48 48" className="w-full h-full drop-shadow" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M10 16 C18 10, 30 10, 38 16 L35 20 C28 15, 20 15, 13 20 Z" fill="url(#op-crust-franchise)"/>
        <path d="M12.5 19.5 C20 14.5, 28 14.5, 35.5 19.5 L24 40 Z" fill="url(#op-cheese-franchise)"/>
        ...
        <defs>
          <linearGradient id="op-crust-franchise" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#F59E0B"/>
            <stop offset="100%" stopColor="#D97706"/>
          </linearGradient>
          <linearGradient id="op-cheese-franchise" x1="0%" y1="0%" x2="100%" y2="100%">
  ...
  if (variant === 'icon') {
    return (
      <div className={`inline-flex items-center justify-center shrink-0 ${className}`} title="Olive Pizza Franchise Management">
        {renderIcon()}
      </div>
    );
  }
  ...
  return (
    <div className={`inline-flex items-center ${currentSize.gap} ${className} shrink-0`}>
      ...
      <div className="flex flex-col justify-center min-w-0">
        ...
        {variant === 'full' && (
          <span className={`${currentSize.sub} font-extrabold text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded-md tracking-wider uppercase inline-block mt-0.5 whitespace-nowrap w-fit`}>
            {subtitle}
          </span>
        )}
      </div>
