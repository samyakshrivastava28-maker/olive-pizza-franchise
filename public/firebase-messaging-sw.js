/**
 * Olive Pizza Franchise — Firebase Cloud Messaging Service Worker
 */
...
firebase.initializeApp({
  ...
  appId: "1:1017239455106:web:5410aa1eafbbe8b8007b9b"
});
...
messaging.onBackgroundMessage(async (payload) => {
  ...
  const notifTitle = payload.notification?.title || data.title || 'Olive Pizza Franchise';
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      ...
      if (clients.openWindow) {
        return clients.openWindow(notifData.url || '/dashboard');
      }
    })
  );
});
